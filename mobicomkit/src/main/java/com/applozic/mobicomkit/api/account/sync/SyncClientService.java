package com.applozic.mobicomkit.api.account.sync;
...
public class SyncClientService extends MobiComKitClientService {
    ...
    public SyncApiResponse getSyncCall(Long syncTime, SyncType syncType) {
        try {
            String response = httpRequestUtils.getResponse(getCredentials(), getSyncUrl() + "?syncTime=" + syncTime + "&syncType=" + syncType.getValue() , "application/json", "application/json");
            if(response != null) {
